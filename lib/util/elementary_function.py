...
## 3rd party libraries
import numpy as np
from scipy.stats import multivariate_normal, t, laplace
...
class HyperbolicSecantMixtureModel(object):
    ...
    @classmethod
    def logpdf(clf, X:np.ndarray, ratio:np.ndarray, mean:np.ndarray, precision:np.ndarray):
        n = X.shape[0]
        K = len(ratio)
...
class StudentMixtureModel(object):
    ...
    @classmethod
    def logpdf(clf, X:np.ndarray, ratio:np.ndarray, mean:np.ndarray, precision:np.ndarray, df:float=1.5):
        n = X.shape[0]
        K = len(ratio)
    ...
    @classmethod
    def rvs(clf, ratio:np.ndarray, mean:np.ndarray, precision:np.ndarray, size:int=1, data_seed:int=-1, df:float = 1.5):
        if data_seed > 0: np.random.seed(data_seed)
        data_label = np.random.multinomial(n = 1, pvals = ratio, size = size)
        data_label_arg = np.argmax(data_label, axis = 1)
...
class LaplaceMixtureModel(object):
    ...
    def logpdf(clf, X:np.ndarray, ratio:np.ndarray, mean:np.ndarray, precision:np.ndarray):
        ...
        for k in range(K):
            if precision.ndim == 2:
                loglik[:,k] = np.log(ratio[k]) + laplace.logpdf(X, loc = mean[k,:], scale = 1/precision[k,:]).sum(axis = 1)
            else:
                raise ValueError("Error precision, dimension of precision must be 2 or 3!")
        max_loglik = loglik.max(axis = 1)
        norm_loglik = loglik - np.repeat(max_loglik,K).reshape(n,K)
